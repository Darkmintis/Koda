...
```js
const context = koda.createBuildContext({referer:"my-referer"});
```
...
- Fix compatibility issue on file media with koda 2.7.x


## 1.0.1

- Make the library generate a .koda file compatible with koda 2.7.x
...
ability to build more than one file at same time and export them in an unique .koda
file.

```js
const context = koda.createBuildContext()
...
const barray = koda.exportAsBytes(context);
```

The previous `file.export()` method has been removed and several alternatives are
added as first level functions on koda library API entrypoint:
...
- Remove `add/update/deleteLibraryMedia` (they are no longer supported by Koda and have been replaced by components)
