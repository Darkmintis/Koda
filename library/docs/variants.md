...
- The first child of the variant container is used to represent visually the entire VariantComponent in different places on Koda, so the order of the children is significant.
- When created in the interface of Koda, it may include some attributes (e.g., flex layout, stroke color `#bb97d8`, border radius `20`). These attributes are not mandatory. (TBD: Will the SDK create those attributes by itself?).
