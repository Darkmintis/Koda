...
desc: Dive into Koda today! Learn about self-hosting, configuration, developer insights, authentication, and more. View Koda's technical guide. Try it free.
---

# User authentication

Users in Koda may register via several different methods (if enabled in the
configuration of the Koda instance). We have implemented this as a series
of "authentication backends" in our code:

 * **koda**: internal registration with email and password.
...
When a user types an email and password in the basic Koda registration page,
...
koda backend, or directly opens a session (see below) for othe methods or if
...
To login with the koda backend, the user simply types the email and password
...
outside of Koda). Typically when LDAP registration is enabled, the plain user
...
```js
var kodaFlags = "enable-login-with-ldap";
```
