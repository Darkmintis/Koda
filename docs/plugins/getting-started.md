...
desc: Dive into Koda plugins! Extend Koda's functionality by automating tasks and adding new features using JavaScript, HTML, & CSS. Get started now!
---
...
Welcome to Koda Plugins!

Plugins are the perfect tool to easily extend Koda's functionality, you can automate repetitive tasks, add new features and much more.

Plugins can be created with your favorite framework or with not framework at all. Feel free to use whatever you want because Plugins are independent from Koda's code and therefore you don't need any extra knowledge.

The plugins will be hosted outside Koda, and each creator need to host theirs.

## 1.2. Pre-requisites

- Basic experience with Koda.
...
![Koda's plugin manager](/img/plugins/plugin-manager.png)
...
| Name          | URL                                                                 |
| ------------- | ------------------------------------------------------------------- |
| Lorem Ipsum   | https://lorem-ipsum-koda-plugin.pages.dev/assets/manifest.json    |
| Contrast      | https://contrast-koda-plugin.pages.dev/assets/manifest.json       |
| Feather icons | https://icons-koda-plugin.pages.dev/assets/manifest.json          |
| Tables        | https://table-koda-plugin.pages.dev/assets/manifest.json          |
| Color palette | https://create-palette-koda-plugin.pages.dev/assets/manifest.json |
| Rename layers | https://rename-layers-koda-plugin.pages.dev/assets/manifest.json  |

## 1.4. Plugin's basics

### How does it work?

Koda's plugin system allows you to add new features to the platform through independent modules called plugins. These plugins run separately from the main Koda app, inside iframes, which are like small, isolated browser windows within the app.

Plugins communicate with Koda by sending and receiving messages through the iframe.
...
Koda_App -down-> WebComponent
WebComponent -up-> Koda_App : Write / Read
...
Plugin_code : koda.ui.open('Example plugin', '');
Plugin_code :
Plugin_code : koda.ui.onMessage((message) => {
...
- **Permissions**: your plugin's permissions, which allow access to different parts of the Koda API.
...
We're using TypeScript to make working with the Koda API easier, as it provides autocompletion and instant access to documentation. However, even with TypeScript’s powerful features, you'll still need to include the <code class="language-js">@koda/plugin-types</code> npm package, which contains the typings for the Koda Plugin API. This ensures that TypeScript can fully understand and work with the API.

![plugin-types example](/img/plugins/plugint-types-example.gif)

You can install the package in any project with <code class="language-js">npm install @koda/plugin-types</code>. You can check the details in [@koda/plugin-types package](https://www.npmjs.com/package/@koda/plugin-types).
