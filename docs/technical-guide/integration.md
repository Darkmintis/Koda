...
desc: Connect Koda with other apps using webhooks and access tokens! Learn from Koda's integration guide for seamless workflows. Try Koda - It's free.
---

# Integration Guide

This section explains how to interconnect Koda with other applications, so
...
Outbound webhooks are event calls from Koda to other applications, that
notifiy some particular event has occured (e.g. a file has been created or
updated, a comment has been added, etc.).

#### Webhooks managament
In Koda, webhooks are configured at Team level-
...
For the moment you can use the <a href="https://design.koda.app/api/_doc"
target="_blank">backend API documentation</a>, generated automatically from <a
href="https://github.com/koda/koda/tree/main/backend/src/app/rpc"
...
Generate a site URL and set it into Koda. Then you can inspect the calls received.


## Access tokens

Personal access tokens function like an alternative to our login/password authentication system and can be used to allow an application to access the internal Koda API.
...
In Koda, access tokens are configured at user account level. To manage your access tokens, go to Your account > Access tokens.
...
This is an example of a curl command that you can run at the console to access your Koda profile using an access token:

```bash
curl -H "Authorization: Token <replace-this-with-token>" https://design.koda.app/api/rpc/command/get-profile
```
